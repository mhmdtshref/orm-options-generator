      ...
      currentFiltersKeys.forEach((currentFilterKey) => {
        if (isLogicalOperators(currentFilterKey)) {
          if (isArray(filter[filterKey][currentFilterKey])) {
            (filter[filterKey][currentFilterKey] as any[]).forEach((logicalOperatorFilter) =>
              filterValidator(logicalOperatorFilter),
            );
          } else {
...
const orderValidator = (order: any) => {
  ...
  orderKeys.forEach((orderKey) => {
    const isRightOrderValue =
      isString(order[orderKey]) && orderValues.includes((order[orderKey] as string).toLowerCase());
